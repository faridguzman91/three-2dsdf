...
void main() {
  ...
  vec2 uv = rotate(vUv - vec2(0.5), uLevel*2.*PI);
  
 /*  float scene = sdCircle(uv, 0.2);  */
/* float scene = sdBox(uv, vec2(0.2)); */


  //formula for color fading
/* color(t) = a + b ⋅ cos[2π(c⋅t+d)] */


//noice call, multiply or add any num
float nnn = cnoise(vec3(uv*10., time*6.));

// throw random numbers at sin to form shapes
float scene = mix(
    sdCircle(uv, 0.2),
    sdBox(uv, vec2(0.2)),
...
//calculate the vertices  distance in the loop
  float dist = length(cameraPosition - vWorldPosition);
  //calculate fading time
